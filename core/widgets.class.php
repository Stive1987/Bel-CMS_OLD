...
final class widgets
{
	...
	public function getGlobalView ()
	{
		...
		$defaultFile = ROOT.$this->widgetsDir.'view.tpl.php';
		$customFile  = $this->dirTemplate.'widgets/view.tpl.php';
...
class GetWidgetsList
{
	...
	public function getWidgetsActive ()
	{
		...
		foreach (Common::scanDirectory(ROOT.$this->widgetsDir) as $name) {
			if (array_key_exists($name, $returnListWidgetsSql)) {
				if ($returnListWidgetsSql[$name]['access']) {
					$arrayListWidgets[$name] = array(
						...
						'fileView'       => ROOT.$this->widgetsDir.$name.'/view.tpl.php',
	...
	protected function getDataSqlWidgets ()
	{
		$return = array();
		$datas = array(
			'fields' => array('id','name','activate','groups','positioning','sorts','cache'),
			'table'  => TABLE_WIDGETS,
			'order'  => ' sorts DESC'
		);
